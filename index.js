...
function languageSelector(message) {
  if (message === fa.languageText) lang = fa;
  else if (message === en.languageText) lang = en;
  else return false;
  keyboardMarkups = {
...
function showKbPrompt(msg, prompt) {
  let replyMarkup = bot.keyboard(
    keyboardMarkups[state] || undefined,
    defaultOptions
  );
...
async function flightInfoQuery(msg, flightObj) {
  let sqlQuery =
    "SELECT * FROM airlineinfo WHERE flightnumber = " +
    connection.escape(flightObj.number) +
    " AND " +
    "flighttype = " +
    connection.escape(flightObj.mode) +
    " AND " +
    "dateflight = " +
    connection.escape(flightObj.date);
  ...
  await connection.query(sqlQuery, (error, rows) => {
    ...
    if (rows[0])
      bot.sendMessage(msg.from.id, flightInfoResponse(rows), {
        parseMode: "Markdown"
      });
    else bot.sendMessage(msg.from.id, lang.flight.notFound);
  });
...
function flightInfoResponse(dbRow) {
  ...
  return (
    ...
    lang.flight.type +
    ": *" +
    (flighttype === "ARR" ? lang.flight.arrival : lang.flight.departure) +
    "\n" +
    "*" +
    lang.flight.date +
    ": *" +
    ...
    "\n" +
    "*" +
    lang.flight.airlineName +
    " *" +
    airline +
    "\n" +
    "*" +
    (flighttype === "ARR" ? lang.flight.from : lang.flight.destination) +
    " *" +
    (lang === en ? destinationen : destinationfa) +
    "\n" +
    "*" +
    lang.flight.scheduled +
    " *" +
    scheduled +
    "\n" +
    "*" +
    ...
    lang.flight.remarks +
    " *" +
    (lang === en ? remarksen : remarksfa) +
    "\n" +
    "*" +
    lang.flight.status +
    " *" +
    (lang === en ? statusen : statusfa) +
    "\n" +
    finalPart
...
bot.on(["/start"], msg => {
  let replyMarkup = bot.keyboard(
    [[en.languageText, fa.languageText]],
    defaultOptions
  );
